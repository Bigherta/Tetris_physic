...
js/constants.js         全局配置: 网格 / 物理 / 材质(含石块) / 计分 / RL 参数 / 动作枚举 / 下落间隔
...
- **石块材质**：每次生成的方块有 **1/10 概率**为石块（专属灰岩外观：灰渐变 + 深色麻点 + 裂纹，与 7 种形状色不冲突）；石块密度更大（`STONE_DENSITY = 0.012`，普通 `0.002`，即 **6× 重**），落地冲击更强、更难被推动。材质参数集中在 `constants.js` 的 `MATERIALS` / `STONE` / `STONE_PROB`，转物理时由 `releaseActive` 用 `Body.setDensity` 据刚体自带的 `matProps` 重新设置质量（不会把石块退回普通重量）。
